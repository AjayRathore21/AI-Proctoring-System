...
import type { InterviewStats, EventLogEntry, EventSeverity } from "../../../types";
...
const InterviewSidebar: React.FC<InterviewSidebarProps> = ({ stats }) => {
  ...
  const getEventIcon = (eventType: string) => {
    ...
    if (eventType.includes("paper") || eventType.includes("note")) {
      return (
      ...
      <div className="p-4 border-b border-blue-500/20 bg-gradient-to-r from-blue-900/20 to-transparent">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs text-white/60 uppercase tracking-wider font-medium">Candidate</p>
            <p className="text-white font-semibold truncate">{stats.candidateName.toUpperCase()}</p>
          </div>
        </div>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-xs text-white/70 font-mono">{event.timestamp}</span>
                    {event.severity === "alert" && (
                      <span className="text-xs text-red-400 font-semibold uppercase">Alert</span>
                    )}
                  </div>
      ...
      <div className="p-4 bg-gradient-to-r from-blue-900/20 to-transparent">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="flex flex-col items-center p-3 rounded-lg bg-white/5 border border-white/10">
            ...
            <p className="text-xs text-white/70 text-center mb-1">Mobile Phone</p>
            <p className="text-sm text-white font-semibold">({stats.itemDetection.mobilePhone})</p>
          </div>
          ...
          <div className="flex flex-col items-center p-3 rounded-lg bg-white/5 border border-white/10">
            ...
            <p className="text-xs text-white/70 text-center mb-1">Notes/Books</p>
            <p className="text-sm text-white font-semibold">({stats.itemDetection.notesBooks})</p>
          </div>
          ...
          <div className="flex flex-col items-center p-3 rounded-lg bg-white/5 border border-white/10">
            ...
            <p className="text-xs text-white/70 text-center mb-1">Extra Electronics</p>
            <p className="text-sm text-white font-semibold">({stats.itemDetection.extraElectronics})</p>
          </div>
          ...
          <div className="flex flex-col items-center p-3 rounded-lg bg-white/5 border border-white/10">
            ...
            <p className="text-sm text-white font-semibold">({stats.itemDetection.smartwatch})</p>
          </div>
        </div>
      </div>
    </div>
